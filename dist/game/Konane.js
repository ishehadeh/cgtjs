    ...
    *movesRight() {
    ...
    cloneWithSize(newW, newH) {
        ...
        if (this.#black !== 0n || this.#white !== 0n) {
            for (let y = 0n; y < this.height; ++y) {
                newGame.#black = BigInt.asUintN(Number(newWb * newHb), newGame.#black | (((this.#black & this.rowMask(y)) >> (y * this.width)) << (y * newWb)));
                newGame.#white = BigInt.asUintN(Number(newWb * newHb), newGame.#white | (((this.#white & this.rowMask(y)) >> (y * this.width)) << (y * newWb)));
            }
        }
        return newGame;
    }
