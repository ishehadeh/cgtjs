function wordCountFromCells(cells) {
    return (cells + 31) >>> 5;
}
/** Clear bits beyond `cellCount - 1` in the backing store. */
function maskWordsToCellCount(words, cellCount) {
...
export class BitBoard {
     ...
     */
    static fromLinearPacked(w, h, packed) {
        const bb = new BitBoard(w, h);
        const cc = w * h;
        for (let i = 0; i < cc; i++) {
            if ((packed >>> i) & 1) {
    ...
    static fromPackedBytes(width, height, dataView, byteOffset, fieldSizeBytes) {
    ...
    writePackedBytes(dataView, byteOffset, fieldSizeBytes) {
 ...
 */
export function deltaSwap(board, mask, delta) {
